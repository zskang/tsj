...
mac.tabs = function(self, cfg) {
	...
	var hl = hx.clone().addClass('left'), hr = hx.clone().addClass('right_');
	...
	self.addTab = function(p, n, cb) {
		...
		if (p.update && a.length > 0) {
			...
			a = $('<div class="item normal_" name="' + k + '"></div>');
			b = $('<iframe class="main hidden iframe_" id="if_'+k+'" name="' + k + '" frameborder="0"></iframe>');
			if (p.bodyCls)
		...
		a.click(function() {
			...
			if (p.url && !b.html() && !p.autoLoad) {
				//modify by huzd for sitech iportal
				$("#if_"+k).attr("src",p.url).attr("style","height:"+($(".rightContainer>.body").height()-5)+"px!important");
				var targetIframe_ = document.getElementById("if_"+k);
				targetIframe_.onload = targetIframe_.onreadystatechange = function() {
				     if (this.readyState && this.readyState != 'complete'){return;}else{if (cfg.onLoadPage){cfg.onLoadPage(self, a, b, p);}}
				};
...
//				});
		...
		if (p.el) {
			...
			var targetIframe_ = document.getElementById("if_"+k);
			targetIframe_.onload = targetIframe_.onreadystatechange = function() {
			     if (this.readyState && this.readyState != 'complete'){return;}else{if (cfg.onLoadPage){
			    	 cfg.onLoadPage(self, a, b, p);
			    	 $("#if_"+k).attr("style","height:"+($(".rightContainer>.body").height()-5)+"px!important");
			     }}
			};			
//			b.load(p.url, p.params, function() {
//				if (cfg.onLoadPage)
