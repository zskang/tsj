 ...
 * 
 * Author: Wes Nolte http://twitter.com/wesnolte
 * 
 * Based on the work of Mark Lee http://www.capricasoftware.co.uk
 * 
 * Copyright (c) 2011 Wesley Nolte Dual licensed under the MIT and GPL licenses.
 * 
 */
(function($) {
	...
	function buildNode($node, $appendTo, level, opts) {
		...
		// Increaments the node count which is used to link the source list and
		// the org chart
		nodeCount++;
		$node.data("tree-node", nodeCount);
		if(level == 0){
			$nodeDiv = $("<div>").addClass("node level_"+level).data("tree-node", nodeCount).append($nodeContent);
		}else{
			var _match_ = /aa.*(?=bb)/;
			var _matchContent =$nodeContent.match(_match_)+'';
			$nodeDiv = $("<div>").addClass("node level_"+level+" height_"+_matchContent.match(/\d+/g)[0]).data("tree-node", nodeCount).append($nodeContent);
		}
		...
		if ($childNodes.length > 0) {
			$nodeDiv.click(function() {
				...
				if ($tr.hasClass('contracted')) {
					...
					// Update the <li> appropriately so that if the tree redraws
					// collapsed/non-collapsed nodes
