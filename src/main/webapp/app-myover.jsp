...
<body>
	<div class="rightinfo">
		<div class="condition">
			<div class="cond_item">
				<input id="moduleId" value="<%=moduleId%>" type="hidden" /> 任务名称：<input
					type="text" class="dfinput small" id="queryTaskName"
					name="queryTaskName" />
			</div>
	<!-- 		<div class="cond_item">
			...
			<div class="cond_item">
				<input type="hidden" name="projectId" id="projectId"
					value="<s:property value='#session.project.id'/>" />
				...
				$
						.each(
								...
								function(i, n) {
									...
									innerHtml += ("<td>" + n.startTime + "</td>");
									innerHtml += ("<td>" + n.endTime2 + "</td>");
									innerHtml += ("<td>" + n.name + "</td>");
									innerHtml += ("<td>"
											+ " <a href='javascript:openListCommentDialog("
											+ n.id
											+ ")'> <img src='${_theme_}btnIcon/detail.png' title='查看操作历史'/></a> <a  target='_blank' href='${basePath}showViewAction.huzd?taskId="
											+ n.id + "'><img src='${_theme_}btnIcon/detail.png' style='width:30;height:30px'  title='查看流程操作轨迹'/></a></td></tr>");
								});
			} else {
				if (data.message)
					art.dialog({
